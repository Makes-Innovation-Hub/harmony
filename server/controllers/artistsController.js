...
const artistImage = async (artistName) => {
    let artistImage;
    axios
        .get(`http://localhost:5000/api/v1/cover/${artistName}`)
        .then((res) => {
            return;
        })
        .catch((error) => {
...
const findOrCreateArtist = async (name, album) => {
    ...
    logger.info(
        `start findOrCreateArtist with data ${name} and album: ${album}`
    );
    const newArtistObject = createObjectFromQuery(data);
    const newArtist = await Artist.create(newArtistObject);
    logger.info(
        `findOrCreateArtist with artist data: ${JSON.stringify(
            newArtist
        )} Created`
    );
    return newArtist;
